...
public class SpeedFly extends Module {
    ...
    public SpeedFly () {
        super("Speed Fly", "Nyoooom!", 71, new SettingTextWidget("Speed", "Changes how fast you nyoom", DEFAULT_SPEED));
    }
    ...
    public void tick(ClientWorld cw) {
        ...
        if(jumpKey) {
            if (leftKey) {
                ...
                newVelocity = new Vec3d(newVelocity.x, 0, newVelocity.z).normalize().multiply(SPEED);
            }
            if (rightKey) {
                newVelocity = newVelocity.add(mc.player.getRotationVector().rotateY(-3.141592f));
                newVelocity = new Vec3d(newVelocity.x, 0, newVelocity.z).normalize().multiply(SPEED);
            ...
            if (forwardKey)
                newVelocity = newVelocity.add(mc.player.getRotationVector().multiply(SPEED));
            if (backKey)
                newVelocity = newVelocity.add(mc.player.getRotationVector().negate().multiply(SPEED));
